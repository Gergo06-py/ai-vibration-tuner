...
export default function Index() {
  ...
  const prompt = 'Your job is to create the perfect vibration method for a phone\'s short button press vibration using timing in the following format: ' +
    '[' +
    '{' +
    '"option1": {' +
    '"vibrations": [v1, v2, ...],' +
  ...
  function setPreferredOption(option: string, feedback?: string) {
    ...
    setCurrentResult({ ...currentResult, preferred: option, feedback: feedback });
  ...
  useEffect(() => {
    if (currentResult.preferred !== "" && currentResult.preferred !== undefined) {
  ...
  const formatOptionOutput = (result: Result) => {
    ...
    const option1Output = `Option 1: Vibrations: ${option1.vibrations.join(', ')}, Delays: ${option1.delays.join(', ')}`;
    const option2Output = `Option 2: Vibrations: ${option2.vibrations.join(', ')}, Delays: ${option2.delays.join(', ')}`;
